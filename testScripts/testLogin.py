        ...
        for idx, i in enumerate(isExecuteUser[1:]):
            ...
            if i == 'Y':
                ...
                try:
                    ...
                    logger.info('登录操作执行...')
                    try:
                        #homePage.ExitButtonObj()  # 如果在首页页面找到退出按钮，则通过测试用例，如果没找到该按钮则测试用例未通过
                        browser.implicitly_wait(5)
                        # homePage.UserInfoObj().click()
                        self.assertIs(homePage.OfficeHomeButton().is_displayed(), True)
                        logger.info('在首页页面找【退出】按钮')
                    except Exception as e:
                        # self.assertTrue(1 == 2)
                        logger.debug('在首页页面找不到【退出】按钮，失败，用例不通过')
                        TestLogin.excelObj.write_cell_value(idx + 2, execute_testResult, 'fail', 'red')
                        TestLogin.excelObj.write_cell_value(idx + 2, execute_time, str(time.time()-start_time)+'s', 'red')
                        raise e
